...
public class Functional {
    ...
    static class AAdder implements Combiner<A> {
        @Override
        public A combine(A x, A y) {
            return new A(x.getValue() + y.getValue());
        }
    }

    static class BAdder implements Combiner<B> {
        @Override
        public B combine(B x, B y) {
            return new B(x.getValue() + y.getValue());
        }
    }

    static class BSubstracter implements Combiner<B> {
        @Override
        public B combine(B x, B y) {
            return new B(x.getValue() - y.getValue());
        }
    ...
    static class GreaterThanZero implements UnaryPredicate<B> {
        @Override
        public boolean test(B x) {
            return x.getValue() > 0;
        }
    }
    ...
    public static void main(String[] args) {
        List<B> lb = Arrays.asList(
                new B(1), new B(2),
                new B(3), new B(4), new B(0)
        );
        System.out.println(filter(lb, new GreaterThanZero()));
        ...
        System.out.println(forEach(li,
                new MultiplyIntegerCollector()).result());
